...
<div id="la_compare_table_wrapper">
    ...
    $lists = Omens_WooCommerce_Compare::get_data();

    $default_attr = Omens_WooCommerce_Compare::get_default_attributes();
    ...
    add_filter('omens/filter/enable_image_lazyload', '__return_false');
    ...
    if (!empty($lists)) {
        foreach ($lists as $product_id) {
            ...
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : esc_html__('N/A', 'omens'));
        ...
        <table class="la-compare-table la-compare-table-items <?php if($total_product > 3){ echo 'show-vtb'; } ?> ">
            ...
            foreach( $attribute_allow as $lk => $lb ){
                ...
                if(in_array($lk, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'omens');
                }
                else{
                ...
                foreach ($terms as $item) {
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($item['stock']) {
                                $text_stock = esc_html__('In stock', 'omens');
                                $class_stock = 'in-stock';
                            } else {
                                $text_stock = esc_html__('Out of stock', 'omens');
        ...
        ?>
        <p class="text-center"><?php esc_html_e('No products were added to the list', 'omens') ?></p>
        <?php
    }
    ?>
</div>
