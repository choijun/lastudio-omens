...
if(!defined('OMENS_THEME_VERSION')){
    define('OMENS_THEME_VERSION', '1.0.0');
}

if(!class_exists('Omens_Theme_Class')){

    final class Omens_Theme_Class {
        ...
        public function theme_setup() {
            ...
            $ext = apply_filters('omens/use_minify_css_file', false) || ( defined('WP_DEBUG') && WP_DEBUG ) ? '' : '.min';

            // Load text domain
            load_theme_textdomain( 'omens', self::$template_dir_path .'/languages' );
            ...
            register_nav_menus( array(
                'main-nav'   => esc_attr_x( 'Main Navigation', 'admin-view', 'omens' )
            ) );
             ...
             */
            add_theme_support( 'custom-header', apply_filters( 'omens/filter/custom_header_args', array(
                'width'              => 2000,
                'height'             => 1200,
                'flex-height'        => true,
                'video'              => true,
            ...
            add_theme_support( 'editor-color-palette', array(
                ...
                array(
                    'name' => esc_attr_x( 'pale pink', 'admin-view', 'omens' ),
                    'slug' => 'pale-pink',
                    'color' => '#f78da7',
                ),

                array(
                    'name' => esc_attr_x( 'theme primary', 'admin-view', 'omens' ),
                    'slug' => 'omens-theme-primary',
                    'color' => '#BC8157',
                ),

                array(
                    'name' => esc_attr_x( 'theme secondary', 'admin-view', 'omens' ),
                    'slug' => 'omens-theme-secondary',
                    'color' => '#212121',
                ),

                array(
                    'name' => esc_attr_x( 'strong magenta', 'admin-view', 'omens' ),
                    'slug' => 'strong-magenta',
                    'color' => '#a156b4',
                ),
                array(
                    'name' => esc_attr_x( 'light grayish magenta', 'admin-view', 'omens' ),
                    'slug' => 'light-grayish-magenta',
                    'color' => '#d0a5db',
                ),
                array(
                    'name' => esc_attr_x( 'very light gray', 'admin-view',  'omens' ),
                    'slug' => 'very-light-gray',
                    'color' => '#eee',
                ),
                array(
                    'name' => esc_attr_x( 'very dark gray', 'admin-view', 'omens' ),
                    'slug' => 'very-dark-gray',
                    'color' => '#444',
                ),
            ) );
        ...
        public function meta_viewport() {
            ...
            echo apply_filters( 'omens_meta_viewport', $viewport );
        ...
        public function theme_css() {
            ...
            $inline_handler_name = 'omens-theme';

            if(function_exists('WC')){
                wp_enqueue_style( 'omens-woocommerce', get_theme_file_uri( '/assets/css/woocommerce'.$ext.'.css' ), false, $theme_version );
                $inline_handler_name = 'omens-woocommerce';
            }
        ...
        public function theme_js() {
            ...
            $ext = !apply_filters('omens/use_minify_js_file', true) || ( defined('WP_DEBUG') && WP_DEBUG ) ? '' : '.min';
            ...
            if(function_exists('WC')){
                $dependencies[] = 'omens-woocommerce';
            }

            $dependencies = apply_filters('omens/filter/js_dependencies', $dependencies);

            wp_enqueue_script('omens-theme', get_theme_file_uri( '/assets/js/app'.$ext.'.js' ), $dependencies, $theme_version, true);

            if (is_singular() && comments_open() && get_option('thread_comments')) {
                wp_enqueue_script('comment-reply');
            }

            if(apply_filters('omens/filter/force_enqueue_js_external', true)){
                wp_localize_script('omens-theme', 'la_theme_config', $localize_array );
            }

            if(function_exists('la_get_polyfill_inline')){

                $polyfill_data = apply_filters('omens/filter/js_polyfill_data', [
                    'omens-polyfill-object-assign' => [
                        'condition' => '\'function\'==typeof Object.assign',
                        'src'       => get_theme_file_uri( '/assets/js/lib/polyfill-object-assign'.$ext.'.js' ),
                        'version'   => $theme_version,
                    ],
                    'omens-polyfill-css-vars' => [
                ...
                if(!empty($polyfill_inline)){
                    wp_add_inline_script('omens-theme', $polyfill_inline, 'before');
                }
            }
        ...
        public function localize_array() {
            ...
            $array = array(
                ...
                'i18n' => array(
                    ...
                    'compare' => array(
                        'view' => esc_attr_x('View List Compare', 'front-view', 'omens'),
                        'success' => esc_attr_x('has been added to comparison list.', 'front-view', 'omens'),
                        'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'omens')
                    ),
                    'wishlist' => array(
                        'view' => esc_attr_x('View List Wishlist', 'front-view', 'omens'),
                        'success' => esc_attr_x('has been added to your wishlist.', 'front-view', 'omens'),
                        'error' => esc_attr_x('An error occurred, Please try again !', 'front-view', 'omens')
                    ),
                    'addcart' => array(
                        'view' => esc_attr_x('View Cart', 'front-view', 'omens'),
                        'success' => esc_attr_x('has been added to your cart', 'front-view', 'omens'),
                        'error' => esc_attr_x('An error occurred, Please try again !', 'front-view', 'omens')
                    ),
                    'global' => array(
                        'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'omens'),
                        'search_not_found' => esc_attr_x('It seems we can’t find what you’re looking for, please try again !', 'front-view', 'omens'),
                        'comment_author' => esc_attr_x('Please enter Name !', 'front-view', 'omens'),
                        'comment_email' => esc_attr_x('Please enter Email Address !', 'front-view', 'omens'),
                        'comment_rating' => esc_attr_x('Please select a rating !', 'front-view', 'omens'),
                        'comment_content' => esc_attr_x('Please enter Comment !', 'front-view', 'omens'),
                        'continue_shopping' => esc_attr_x('Continue Shopping', 'front-view', 'omens'),
                        'cookie_disabled' => esc_attr_x('We are sorry, but this feature is available only if cookies are enabled on your browser', 'front-view', 'omens'),
                        'more_menu' => esc_attr_x('Show More +', 'front-view', 'omens'),
                        'less_menu' => esc_attr_x('Show Less', 'front-view', 'omens'),
                        'search_view_more' => esc_attr_x('View More', 'front-view', 'omens'),
                    )
                ),
                'popup' => array(
                    'max_width' => esc_attr(omens_get_option('popup_max_width', 790)),
                    'max_height' => esc_attr(omens_get_option('popup_max_height', 430))
                ),
                'js_path'       => esc_attr(apply_filters('omens/filter/js_path', self::$template_dir_url . '/assets/js/lib/')),
                'js_min'        => apply_filters('omens/use_minify_js_file', true),
                'theme_path'    => esc_attr(apply_filters('omens/filter/theme_path', self::$template_dir_url . '/')),
            ...
            return apply_filters( 'omens/filter/localize_array', $array );
        }
        ...
        public function register_sidebars() {
            ...
            $heading = 'h4';
            $heading = apply_filters( 'omens/filter/sidebar_heading', $heading );

            // Default Sidebar
            register_sidebar( array(
                'name'			=> esc_html__( 'Default Sidebar', 'omens' ),
                'id'			=> 'sidebar',
                'description'	=> esc_html__( 'Widgets in this area will be displayed in the left or right sidebar area if you choose the Left or Right Sidebar layout.', 'omens' ),
        ...
        public static function enqueue_google_fonts_url(){
            ...
            if ( 'off' !== _x( 'on', 'Poppins font: on or off', 'omens' ) ) {
        ...
        public function register_breakpoint(){
            if(defined('ELEMENTOR_VERSION')){
                $has_register_breakpoint = get_option('omens_has_register_breakpoint', false);
                if(empty($has_register_breakpoint)){
                    ...
                    $default_settings = [
                        ...
                        'system_colors' => [
	                        [
		                        '_id' => 'primary',
		                        'title' => esc_html__( 'Primary', 'omens' )
	                        ],
	                        [
		                        '_id' => 'secondary',
		                        'title' => esc_html__( 'Secondary', 'omens' )
	                        ],
	                        [
		                        '_id' => 'text',
		                        'title' => esc_html__( 'Text', 'omens' )
	                        ],
	                        [
		                        '_id' => 'accent',
		                        'title' => esc_html__( 'Accent', 'omens' )
	                        ]
                        ],
                        'system_typography' => [
	                        [
                    ...
                    update_option('omens_has_register_breakpoint', true);
                }
            }
        }
    }
