    ...
    $(function () {
        ...
        if(typeof pagenow !== "undefined" && pagenow === "widgets"){
            ...
            $create_box.find( '.la_pb_create_widget_area' ).on('click', function( event ) {
                ...
                $.ajax( {
                    type: "POST",
                    url: omens_admin_vars.ajaxurl,
                    data:
                        {
                            action : 'omens_core_action',
                            router : 'add_sidebar',
                            admin_load_nonce : omens_admin_vars.admin_load_nonce,
            ...
            $la_pb_sidebars.each( function() {
                ...
                $(this).closest('.widgets-holder-wrap').find('.sidebar-name h2, .sidebar-name h3').before( '<a href="#" class="la_pb_widget_area_remove">' + omens_admin_vars.delete_string + '</a>' );
                ...
                $( '.la_pb_widget_area_remove' ).on('click' ,function( event ) {
                    ...
                    if(confirm(omens_admin_vars.confirm_delete_string)){
                        $.ajax( {
                            type: "POST",
                            url: omens_admin_vars.ajaxurl,
                            data:
                                {
                                    ...
                                    admin_load_nonce : omens_admin_vars.admin_load_nonce,
                                    widget_area_name : $this_el.closest( '.widgets-holder-wrap' ).find( 'div[id^=omens_widget_area_]' ).attr( 'id' )
                                },
                            success: function( data ){
                                if(data.success){
