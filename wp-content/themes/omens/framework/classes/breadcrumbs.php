<?php
/**
 * Omens Breadcrumb Trail class
 *
 * @package Omens WordPress theme
 * @since 1.0.0
 */

if ( !function_exists('omens_breadcrumb_trail') ) {
    function omens_breadcrumb_trail( $args = array() ) {

        // Return if breadcrumbs are disabled
        if ( ! apply_filters( 'omens/filter/display_breadcrumbs', true ) || is_front_page() ) {
            return;
        }
        ...
        if ( !is_object( $breadcrumb ) )
            $breadcrumb = new Omens_Breadcrumb_Trail( $args );
...
if( !function_exists('omens_sp_breadcrumbs_before') ) {
    function omens_sp_breadcrumbs_before() {
        $classes = 'site-breadcrumbs';
        echo '<div class="'. esc_attr($classes) .'">';
    }
}
add_action( 'seopress_breadcrumbs_before_html', 'omens_sp_breadcrumbs_before' );

if(!function_exists('omens_sp_breadcrumbs_after')){
    function omens_sp_breadcrumbs_after() {
        echo '</div>';
    }
}
add_action( 'seopress_breadcrumbs_after_html', 'omens_sp_breadcrumbs_after' );
...
if( !class_exists('Omens_Breadcrumb_Trail') ) {

    class Omens_Breadcrumb_Trail {
        ...
        /**
         * Sets up the breadcrumb trail properties.  Calls the `Omens_Breadcrumb_Trail::add_items()` method
         ...
         *     @type array     $labels         Text labels. @see Omens_Breadcrumb_Trail::set_labels()
         *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Omens_Breadcrumb_Trail::set_post_taxonomy()
        ...
        public function trail() {
            ...
            echo omens_render_variable( $this->get_trail() );
        }
        ...
        protected function set_labels() {
            ...
            $defaults = array(
                ...
                'archive_minute'      	=> esc_html__( 'Minute %s',                             'omens' ),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week'        	=> esc_html__( 'Week %s',                               'omens' ),
        ...
        protected function add_network_home_link() {
            ...
            if ( is_multisite() && ! is_main_site() && true === $this->args['network'] )
                $this->items[] = sprintf( '<a href="%s" rel="home" aria-label="' .$this->labels['home']. '" %s>%s</a>', esc_url( network_home_url() ), $icon, omens_get_schema_markup('item'), $text );
        }
        ...
        protected function add_minute_hour_archive_items() {
            ...
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'omens' ) ) );
        }
        ...
        protected function add_minute_archive_items() {
            ...
            // Add the minute item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'omens' ) ) );
        }
        ...
        protected function add_hour_archive_items() {
            ...
            // Add the hour item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'omens' ) ) );
        }
        ...
        protected function add_day_archive_items() {
            ...
            $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'omens' ) ) );
