...
if(!class_exists('Omens_WooCommerce_Config')){

    class Omens_WooCommerce_Config {
        ...
        public function __construct() {
            ...
            add_filter('omens/filter/sidebar_primary_name', array( $this, 'set_sidebar_for_shop'), 20 );
        ...
        public function register_sidebars(){
            ...
            register_sidebar( array(
                'name'			=> esc_html__( 'Sidebar - Shop', 'omens' ),
                'id'			=> 'sidebar-shop',
                'description'	=> esc_html__( 'Widgets in this area will be displayed in the shop page.', 'omens' ),
            ...
            register_sidebar( array(
                'name'			=> esc_html_x( 'Sidebar - Filter', 'admin-view',  'omens' ),
        ...
        public function theme_js(){
            ...
            $ext = apply_filters('omens/use_minify_js_file', false) || ( defined('WP_DEBUG') && WP_DEBUG ) ? '' : '.min';

            wp_register_script('omens-woocommerce', get_theme_file_uri( '/assets/js/lib/woocommerce'. $ext .'.js' ), array('jquery'), $theme_version, true);
        ...
        public function add_quick_view_btn(){
            if( 'on' == omens_get_option('woocommerce_show_quickview_btn', 'off') ){
                ...
                printf(
                    ...
                    esc_attr_x('Quick View', 'front-view', 'omens'),
                    esc_attr_x('Quick View', 'front-view', 'omens')
                );
            }
        }
            ...
            if( omens_get_option('woocommerce_show_compare_btn', 'on') == 'on' ) {
                if ( !empty($yith_woocompare->obj) ) {
                    ...
                    printf(
                        ...
                        esc_attr($css_class),
                        esc_url($url),
                        esc_attr_x('Add to compare','front-view', 'omens'),
                    ...
                    $text = esc_html_x('Add to compare','front-view', 'omens');
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s"><span class="labtn-icon labtn-icon-compare"></span><span class="labtn-text">%s</span></a>',
                        esc_attr($css_class),
                        esc_url($url),
            ...
            if(omens_get_option('woocommerce_show_wishlist_btn', 'off') == 'on'){
                ...
                if (function_exists('YITH_WCWL')) {
                    ...
                    if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                        $text = esc_html_x('View Wishlist', 'front-view', 'omens');
                        $class = 'add_wishlist la-yith-wishlist button added';
                        $url = YITH_WCWL()->get_wishlist_url('');
                    }
                    else {
                        $text = esc_html_x('Add to Wishlist', 'front-view', 'omens');
                ...
                elseif(class_exists('TInvWL_Public_AddToWishlist', false)){
                    ...
                    if(Omens_WooCommerce_Wishlist::is_product_in_wishlist($product->get_id())){
                        $class = 'add_wishlist button la-core-wishlist added';
                        $url = omens_get_wishlist_url();
                        $text = esc_html_x('View Wishlist', 'front-view', 'omens');
                    }
                    else{
        ...
        public function set_cookie_default(){
            if (isset($_GET['per_page'])) {
                add_filter('omens/filter/get_product_per_page', array( $this, 'get_parameter_per_page'));
            }
        }

        public function get_parameter_per_page($per_page) {
            if (isset($_GET['per_page']) && ($_per_page = $_GET['per_page'])) {
                $param_allow = omens_woo_get_product_per_page_array();
        ...
        public function add_custom_tabs( $tabs ){
            ...
            if(omens_string_to_bool(omens_get_option('woo_enable_custom_tab'))){
                $custom_tabs = omens_get_option('woo_custom_tabs');
        ...
        public function callback_custom_tab_content( $tab_key, $tab_instance ){
            if(!empty($tab_instance['custom_content'])){
                echo wp_kses_post( omens_transfer_text_to_format($tab_instance['custom_content'], true) );
            }
        }
        ...
        public function add_toolbar_close(){
            if(wc_get_loop_prop('lakit_loop_allow_extra_filters')){
                ...
                if(!empty($per_row_array)){
                    ...
                    echo sprintf('<button><span data-text="%1$s">%1$s</span></button>', esc_html__('View', 'omens'));
                    echo '<ul>';
                    foreach ($per_row_array as $val){
                        echo sprintf('<li><a data-col="%3$s" href="javascript:;">%1$s %2$s</a></li>', esc_html__('View', 'omens'), ($val < 10 ? '0' . $val : $val), esc_html($val));
                    }
                    echo '</ul>';
                    echo '</div>';
                }
                if(!empty($per_page_array)){
                    ...
                    echo sprintf('<button><span data-text="%1$s">%1$s %2$s</span></button>', esc_html__('Show', 'omens'), esc_html($per_page));
                    echo '<ul>';
                    foreach ($per_page_array as $val){
                        echo sprintf('<li %4$s><a href="%3$s">%1$s %2$s</a></li>',
                            esc_html__('Show', 'omens'),
                            esc_html($val),
                ...
                if($active_shop_filter && is_active_sidebar('sidebar-shop-filter')){
                    echo sprintf('<div class="lasf-custom-dropdown wc-custom-filters"><button class="btn-advanced-shop-filter"><span>%1$s</span></button></div>', esc_html__('Filters', 'omens'));
                }
                if( omens_string_to_bool($woocommerce_toggle_grid_list) ):
                    ?>
                    <div class="wc-view-toggle">
                        <button data-view_mode="list"<?php if ($view_mode == 'list') echo ' class="active"';?>><i title="<?php echo esc_attr__('List view', 'omens') ?>" class="lastudioicon-list-bullet-2"></i></button>
                        <button data-view_mode="grid"<?php if ($view_mode == 'grid')  echo ' class="active"'; ?>><i title="<?php echo esc_attr__('Grid view', 'omens') ?>" class="lastudioicon-microsoft"></i></button>
                    </div>
                <?php
                endif;
                ...
                if (omens_string_to_bool($active_shop_filter) && is_active_sidebar('sidebar-shop-filter')):
                ?>
                        ...
                        <div class="sidebar-inner">
                            ...
                            <?php if( is_filtered() || (!is_filtered() && is_product_taxonomy() && isset($_GET['la_doing_ajax'])) ) : ?>
                                <div class="la-advanced-product-filters-result">
                                    <?php
                                    $base_filter = omens_get_base_shop_url();
                                    if(isset($_GET['la_preset'])){
                                        $base_filter = add_query_arg('la_preset', $_GET['la_preset'], $base_filter);
                                    }
                                    ?>
                                    <a class="reset-all-shop-filter" href="<?php echo esc_url($base_filter) ?>"><i class="lastudioicon-e-remove"></i><span><?php echo esc_html__('Clear All Filter', 'omens'); ?></span></a>
                                </div>
                            <?php endif; ?>
                        </div>
