...
if(!function_exists('omens_modify_sale_flash')){
    function omens_modify_sale_flash( $output ){
        return str_replace('class="onsale"', 'class="la-custom-badge onsale"', $output);
    }
}
add_filter('woocommerce_sale_flash', 'omens_modify_sale_flash');

if(!function_exists('omens_modify_product_list_preset')){
    function omens_modify_product_list_preset( $preset ){
        $preset = array(
            '1' => esc_html__( 'Default', 'omens' )
        );
        return $preset;
    }
}
...
if(!function_exists('omens_get_base_shop_url')){
    function omens_get_base_shop_url( ){
...
if(!function_exists('omens_wc_allow_translate_text_in_swatches')){

    function omens_wc_allow_translate_text_in_swatches( $text ){
        return esc_html_x( 'Choose an option', 'front-view', 'omens' );
    }

    add_filter('LaStudio/swatches/args/show_option_none', 'omens_wc_allow_translate_text_in_swatches', 10, 1);
}


if(!function_exists('omens_override_woothumbnail_size_name')){
    function omens_override_woothumbnail_size_name( ) {
        return 'woocommerce_gallery_thumbnail';
    }
    add_filter('woocommerce_gallery_thumbnail_size', 'omens_override_woothumbnail_size_name', 0);
}


if(!function_exists('omens_override_woothumbnail_size')){
    function omens_override_woothumbnail_size( $size ) {
    ...
    add_filter('woocommerce_get_image_size_gallery_thumbnail', 'omens_override_woothumbnail_size');
}

if(!function_exists('omens_override_woothumbnail_single')){
    function omens_override_woothumbnail_single( $size ) {
        if(!function_exists('wc_get_theme_support')){
            return $size;
        }
    ...
    add_filter('woocommerce_get_image_size_single', 'omens_override_woothumbnail_single', 0);
}



if ( !function_exists('omens_modify_text_woocommerce_catalog_orderby') ){
    function omens_modify_text_woocommerce_catalog_orderby( $data ) {
        $data = array(
            'menu_order' => esc_html__( 'Sort by Default', 'omens' ),
            'popularity' => esc_html__( 'Sort by Popularity', 'omens' ),
            'rating'     => esc_html__( 'Sort by Rated', 'omens' ),
            'date'       => esc_html__( 'Sort by Latest', 'omens' ),
            'price'      => sprintf( wp_kses( __( 'Sort by Price: %s', 'omens' ), array( 'i' => array( 'class' => array() ) ) ), '<i class="lastudioicon-arrow-up"></i>' ),
            'price-desc' => sprintf( wp_kses( __( 'Sort by Price: %s', 'omens' ), array( 'i' => array( 'class' => array() ) ) ), '<i class="lastudioicon-arrow-down"></i>' ),
        );
        return $data;
    }

    add_filter('woocommerce_catalog_orderby', 'omens_modify_text_woocommerce_catalog_orderby');
}
...
if(!function_exists('omens_add_custom_block_to_cart_page')){
    function omens_add_custom_block_to_cart_page(){
        ...
        <div class="lasf-extra-cart lasf-extra-cart--calc">
            <h2><?php esc_html_e('Estimate Shipping', 'omens'); ?></h2>
            <p><?php esc_html_e('Enter your destination to get shipping', 'omens'); ?></p>
            <div class="lasf-extra-cart-box"></div>
        </div>
        <div class="lasf-extra-cart lasf-extra-cart--coupon">
            <h2><?php esc_html_e('Discount code', 'omens'); ?></h2>
            <p><?php esc_html_e('Enter your coupon if you have one', 'omens'); ?></p>
            <div class="lasf-extra-cart-box"></div>
        </div>
        <?php
    }
    add_action('woocommerce_cart_collaterals', 'omens_add_custom_block_to_cart_page', 5);
}


if(!function_exists('omens_add_custom_heading_to_checkout_order_review')){
    function omens_add_custom_heading_to_checkout_order_review(){
        ?><h3 id="order_review_heading_ref"><?php esc_html_e( 'Your order', 'omens' ); ?></h3><?php
    }
}
add_action('woocommerce_checkout_order_review', 'omens_add_custom_heading_to_checkout_order_review', 0);
...
if( !function_exists('omens_calculator_free_shipping_thresholds')){
    ...
    add_action( 'woocommerce_widget_shopping_cart_before_buttons', 'omens_calculator_free_shipping_thresholds', 5 );
    add_action( 'woocommerce_before_cart_table', 'omens_calculator_free_shipping_thresholds', 5 );
}

if(!function_exists('omens_wc_add_qty_control_plus')){
    function omens_wc_add_qty_control_plus(){
        echo '<span class="qty-plus"><i class="lastudioicon-i-add-2"></i></span>';
    }
}

if(!function_exists('omens_wc_add_qty_control_minus')){
    function omens_wc_add_qty_control_minus(){
        echo '<span class="qty-minus"><i class="lastudioicon-i-delete-2"></i></span>';
    }
}

add_action('woocommerce_after_quantity_input_field', 'omens_wc_add_qty_control_plus');
add_action('woocommerce_before_quantity_input_field', 'omens_wc_add_qty_control_minus');

if(!function_exists('omens_wc_product_bulk_edit_start')){
    function omens_wc_product_bulk_edit_start(){
        ...
        LASF::field(array(
            ...
            'type'      => 'button_set',
            'default'   => 'nochange',
            'title'     => esc_html_x('Enable Custom Badges', 'admin-view', 'omens'),
            'options'   => array(
                'nochange'          => esc_html_x('No Change', 'admin-view', 'omens'),
                'addnew'            => esc_html_x('Override', 'admin-view', 'omens'),
                'removeall'         => esc_html_x('Remove Existing Data', 'admin-view', 'omens')
            )
        ), '', 'la_custom_badge');
        LASF::field(array(
            'id'                => 'product_badges',
            'type'              => 'group',
            'title'             => esc_html_x('Custom Badges', 'admin-view', 'omens'),
            'button_title'      => esc_html_x('Add Badge','admin-view', 'omens'),
            'max'               => 3,
            ...
            'fields'            => array(
                array(
                    ...
                    'type'          => 'text',
                    'default'       => 'New',
                    'title'         => esc_html_x('Badge Text', 'admin-view', 'omens')
                ),
                array(
                    'id'            => 'bg',
                    'type'          => 'color',
                    'default'       => '',
                    'title'         => esc_html_x('Custom Badge Background Color', 'admin-view', 'omens')
                ),
                array(
                    'id'            => 'color',
                    'type'          => 'color',
                    'default'       => '',
                    'title'         => esc_html_x('Custom Badge Text Color', 'admin-view', 'omens')
                ),
                array(
                    'id'            => 'el_class',
                    'type'          => 'text',
                    'default'       => '',
                    'title'         => esc_html_x('Extra CSS class for badge', 'admin-view', 'omens')
                )
            )
        ), '', 'la_custom_badge');
        echo '</div></div>';
...
add_action( 'woocommerce_product_bulk_edit_start', 'omens_wc_product_bulk_edit_start' );

if(!function_exists('omens_wc_product_bulk_edit_save')){
...
add_action( 'woocommerce_product_bulk_edit_save', 'omens_wc_product_bulk_edit_save' );

if(!function_exists('omens_wc_swatches_attribute_change_type_default')){
    function omens_wc_swatches_attribute_change_type_default( $value, $product_id, $is_tax, $product ){
        if($is_tax){
            $value = 'term_options';
        }
        return $value;
...
add_filter( 'LaStudio/swatches/attribute/default_type', 'omens_wc_swatches_attribute_change_type_default', 10, 4);

if(!function_exists('omens_wc_disable_redirect_single_search_result')){
    function omens_wc_disable_redirect_single_search_result( $value ){
...
add_filter('woocommerce_redirect_single_search_result', 'omens_wc_disable_redirect_single_search_result');

if(!function_exists('omens_wc_render_variation_templates')){
    function omens_wc_render_variation_templates(){
