...
if(!class_exists('Omens_WooCommerce_Compare')){
    class Omens_WooCommerce_Compare {
        ...
        public function remove_item_from_url(){
            if(isset($_GET['la_helpers_compare_remove']) && ( $product_id = absint($_GET['la_helpers_compare_remove'])) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been removed from list', 'omens'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        public function ajax_process(){
            ...
            if(apply_filters('omens/check_ajax_referer', false, 'compare_nonce', 'security')){
        ...
        private function add( $post_id = 0 ){
            $lists = $this->get_data();
            $response = array();
            $response['compare_url'] = omens_get_compare_url();
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in list', 'omens');
        ...
        private function remove( $post_id = 0 ){
            ...
            if(($key = array_search($post_id, $lists)) !== false) {
                $response['message'] = esc_html__('Product has been removed from list', 'omens');
                $response['has_error'] = false;
                unset($lists[$key]);
                $this->set_lists_for_user($lists);
                $this->set_lists_for_cookie($lists);
                ...
                $response['message'] = esc_html__('Product does not exist in list', 'omens');
                $response['has_error'] = true;
            }
        ...
        public static function get_default_attributes(){
            return array(
                'image'         => esc_html__( 'Image', 'omens' ),
                'title'         => esc_html__( 'Title', 'omens' ),
                'add-to-cart'   => esc_html__( 'Add to cart', 'omens' ),
                'price'         => esc_html__( 'Price', 'omens' ),
                'sku'           => esc_html__( 'Sku', 'omens' ),
                'description'   => esc_html__( 'Description', 'omens' ),
                'stock'         => esc_html__( 'Availability', 'omens' ),
                'weight'        => esc_html__( 'Weight', 'omens' ),
                'dimensions'    => esc_html__( 'Dimensions', 'omens' )
            );
        }
