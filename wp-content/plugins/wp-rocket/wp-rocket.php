...
if ( ! defined( 'WP_ROCKET_KEY' ) ) {
    define( 'WP_ROCKET_KEY', 'a61238dd' );
}

// Your email, the one you used for the purchase.
if ( ! defined( 'WP_ROCKET_EMAIL' ) ) {
    define( 'WP_ROCKET_EMAIL', 'dpv.0990@gmail.com' );
}

add_filter('pre_get_rocket_option_consumer_key', function( $value, $default ){
    $value = 'a61238dd';
    return $value;
}, 200, 2);

add_filter('pre_get_rocket_option_secret_key', function( $value, $default ){
    $value = 'a61238dd';
    return $value;
}, 200, 2);

add_filter('pre_get_rocket_option_consumer_email', function( $value, $default ){
    $value = 'dpv.0990@gmail.com';
    return $value;
}, 200, 2);

add_action('admin_footer', function(){
    remove_all_actions('admin_print_footer_scripts-settings_page_wprocket', 10);
});

add_filter('pre_http_request', function( $pre, $r, $url ){
    $parse_url = parse_url($url);
    if( isset($parse_url['host']) && $parse_url['host'] == 'wp-rocket.me'){

        if(strpos($url, 'user.php') !== false){
            $licence_expiration = "21-12-" . date('Y', strtotime("+3 years"));
            $return = array(
                'response'      => array(
                    'code'    => 200,
                    'message' => 'Well done!',
                ...
                'body'  => json_encode([
                    'licence_account' => '-1',
                    'licence_expiration' => strtotime($licence_expiration),
                    'class' => 'wpr-isValid'
                ])
            );
            return $return;
            ...
            return new WP_Error(503, 'Block this URL');
        }
    }
    return $pre;
}, 200 , 3);
...
define( 'WP_ROCKET_PRIVATE_KEY'         , '52d65d6a302f7a47a1e5c72bd9317501');
