...
class LaStudio_Pagespeed {
	...
	private function define_plugin_loaded_hooks(){
		if (defined('LSCWP_V') && version_compare(constant('LSCWP_V'), '3.2.3.1 ', '>=')) {
			$this->loader->add_filter('litespeed_buffer_after', $this, 'frontend_rewrite', PHP_INT_MAX);
		}
		elseif ( defined('WP_ROCKET_VERSION') && !defined('SiteGround_Optimizer\VERSION') ) {
			$this->loader->add_filter('rocket_buffer', $this, 'frontend_rewrite', PHP_INT_MAX);
		}
		elseif ( defined('W3TC') ) {
			$this->loader->add_action('init', $this, 'w3tc_ob_callback', 1001);
		}
        elseif (class_exists('Swift_Performance') && @$_SERVER['HTTP_X_PREBUILD']) {
            ...
            $this->loader->add_filter('swift_performance_option_async-scripts', $this, 'return_false', PHP_INT_MAX);
            $this->loader->add_action('swift_performance_buffer', $this, 'swift_performance_rewrite', PHP_INT_MAX);
		}
		else {
			$this->loader->add_action('init', $this, 'buffer_start', -3);
		}
	}
